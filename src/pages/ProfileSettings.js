...
export default function ProfileSettings() {
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      setShowReviewPopup(true); // ✅ open review popup immediately after verification
    } catch (err) {
      console.error(err);
      alert("Invalid OTP");
    }
  ...
  const handleSubmitReview = async () => {
    try {
      ...
      setUser(prev => ({ ...prev, hasReviewed: true })); // ✅ prevent future popups
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{
      maxWidth: "450px", margin: "80px 0 0 50px", padding: "20px", borderRadius: "10px",
